...
df = pd.read_csv("res/hist_thr.txt", names=['num_points', 'dim', 'proc', 'time'])
plt.hist(df['time']*1000, bins=20)
plt.title("Distribution of execution time for 16 processes for same pivot and dataset", fontsize=20)
...
"""
df = pd.read_csv("./res/hist_vs.txt", names=['num_points', 'dim', 'proc', 'time'])
val = df.proc.unique()
counter = 0
for i in val:
